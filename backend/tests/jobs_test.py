...
async def setup_questionnaire(db_session: AsyncSession) -> Questionnaire:
    q = Questionnaire(
        title="Default Soft Skills Questionnaire", description="Standard assessment"
    )
    db_session.add(q)
    await db_session.commit()
    await db_session.refresh(q)

    dimensions = ["Comunicação", "Liderança", "Adaptabilidade", "Trabalho em Equipe"]
    for dim in dimensions:
        q_item = Question(
            questionnaire_id=q.id, dimension=dim, text=f"Avalie sua habilidade em {dim}"
        )
...
async def test_full_impact_analysis_flow(
    ...
    job_resp = await authenticated_client.post("/api/v1/jobs", json=job_payload)
    assert job_resp.status_code == 201
    job_id = job_resp.json()["id"]

    # 3. Authenticate Candidate via Google
    cand_auth = await authenticated_client.post(
        "/api/v1/auth/google",
        json={"id_token": "mock_google_token_cand1", "job_id": job_id},
    )
    assert cand_auth.status_code == 200
    cand_token = cand_auth.json()["access_token"]
    candidate_id = cand_auth.json()["user"]["id"]

    # 4. Candidate submits answers to questionnaire
    stmt = select(Question).where(Question.questionnaire_id == setup_questionnaire.id)
    ...
    assert cand_list[0]["soft_skills_completed"] is True
    assert cand_list[0]["fit_score"] is not None

    # 6. Recruiter runs Impact Analysis & Post-Hiring Simulation
    impact_resp = await authenticated_client.post(
        f"/api/v1/jobs/{job_id}/candidates/{candidate_id}/impact-analysis"
    )
    assert impact_resp.status_code == 200
    impact_data = impact_resp.json()

    assert impact_data["job_id"] == job_id
    assert impact_data["candidate_id"] == candidate_id
